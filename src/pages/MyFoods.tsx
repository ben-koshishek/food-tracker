...
function normalizedProductToSavedFood(
...
export function MyFoods() {
    ...
    const handleBarcodeSearch = async () => {
        ...
        try {
            ...
            if (product) {
                ...
                setNewServingSize(
                    (product.servingUnitWeight || product.servingQuantity || 100).toString(),
                );
                setBarcode('');
            } else {
    ...
    const handleLogFood = async () => {
        ...
        const servingGrams =
            getGramsFromServing(selectedFood, servingSize, useServingUnit) ||
        ...
        const displayUnit =
            useServingUnit && selectedFood.servingUnitName ? selectedFood.servingUnitName : 'g';
        const displaySize =
            useServingUnit && selectedFood.servingUnitName
    ...
    const selectedFoodPreview = selectedFood
        ? calculatePreview(
              selectedFood,
              getGramsFromServing(selectedFood, servingSize, useServingUnit) || 100,
          )
        : null;
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Scan a barcode or create your own
                            </p>
                        </div>
                        ...
                        {searchQuery || selectedCategory !== 'all' ? (
                            ...
                            <>
                                ...
                                <p className="text-muted-foreground mb-4">
                                    Start by scanning a barcode or looking up a product.
                                </p>
                            </>
                        )}
                    </CardContent>
                    ...
                    {filteredFoods.map((food) => (
                        <Card
                            ...
                            onClick={() => {
                                ...
                                if (food.servingUnitName && food.servingUnitWeight) {
                                    ...
                                    setServingSize(
                                        (food.defaultServingSize / food.servingUnitWeight).toFixed(
                                            1,
                                        ),
                                    );
                                } else {
                        ...
                        >
                            ...
                            <div className="relative h-24 bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center overflow-hidden">
                                ...
                                {food.category && (
                                    <div
                                        className={`absolute top-2 right-2 px-2 py-0.5 text-xs font-medium rounded ${CATEGORY_COLORS[food.category]}`}
                                    >
                            ...
                            <CardContent className="py-3">
                                ...
                                {(food.nutriScoreGrade && food.nutriScoreGrade !== 'unknown') ||
                                food.novaGroup ? (
                                    <div className="flex gap-1.5 mt-2">
                                        {food.nutriScoreGrade && (
                                            <NutriScoreBadge grade={food.nutriScoreGrade} />
                                        )}
                                        {food.novaGroup && (
                                            <NovaGroupBadge group={food.novaGroup} />
                                        )}
                                    </div>
                                ) : null}
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    <Badge
                                        ...
                                        style={{
                                            backgroundColor:
                                                'color-mix(in srgb, var(--macro-calories) 20%, transparent)',
                        ...
                        <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        ...
                                        {selectedFood.nutriScoreGrade && (
                                            <NutriScoreBadge grade={selectedFood.nutriScoreGrade} />
                                        )}
                                        {selectedFood.novaGroup && (
                                            <NovaGroupBadge group={selectedFood.novaGroup} />
                                        )}
                                    </div>
                                </div>
                            </div>
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    {selectedFood.servingUnitName &&
                                        selectedFood.servingUnitWeight && (
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    if (useServingUnit) {
                                ...
                                {useServingUnit && selectedFood.servingUnitWeight && (
                                    <p className="text-xs text-muted-foreground">
                                        ={' '}
                                        {Math.round(
                                            getGramsFromServing(selectedFood, servingSize, true),
                                        )}
                                        g ({selectedFood.servingUnitWeight}g per{' '}
                                        {selectedFood.servingUnitName})
                                    </p>
                                )}
                            </div>
                            {selectedFoodPreview && (
                                <div className="flex gap-2 flex-wrap">
                                    ...
                                    >
                                        {selectedFoodPreview.calories} kcal
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                    ...
                                    >
                                        P: {selectedFoodPreview.protein}g
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                    ...
                                    >
                                        F: {selectedFoodPreview.fat}g
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                            borderColor: 'var(--macro-fiber)',
                                            color: 'var(--macro-fiber)',
                                        }}
                                    >
                                        Fiber: {selectedFoodPreview.fiber}g
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                            borderColor: 'var(--macro-sugar)',
                                            color: 'var(--macro-sugar)',
                                        }}
                                    >
                                        Sugar: {selectedFoodPreview.sugar}g
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                            borderColor: 'var(--macro-salt)',
                                            color: 'var(--macro-salt)',
                                        }}
                                    >
                                        Salt: {selectedFoodPreview.salt}g
                                    </Badge>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        {newProduct.nutriScoreGrade && (
                                            <NutriScoreBadge grade={newProduct.nutriScoreGrade} />
                                        )}
                                        {newProduct.novaGroup && (
                                            <NovaGroupBadge group={newProduct.novaGroup} />
                                        )}
                                    </div>
                                </div>
                            </div>
                            ...
                            {newProductPreview && (
                                <div className="flex gap-2 flex-wrap">
                                    ...
                                    >
                                        {newProductPreview.calories} kcal
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                    ...
                                    >
                                        Fiber: {newProductPreview.fiber}g
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
                                    ...
                                    >
                                        Sugar: {newProductPreview.sugar}g
                                    </Badge>
                                    <Badge
                                        variant="outline"
                                        style={{
