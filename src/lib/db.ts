...
export async function getFoodEntriesForDateRange(
    ...
    return await db.foodEntries.where('date').between(startDate, endDate, true, true).toArray();
}
...
export async function addStorePricing(pricing: Omit<StorePricing, 'id'>): Promise<number> {
    ...
    if (pricing.pricePerKg === undefined) {
        pricing.pricePerKg = calculatePricePerKg(
            pricing.price,
            pricing.packageSize,
            pricing.packageUnit,
        );
...
export async function updateStorePricing(
    ...
    if (
        ...
        pricing.packageSize !== undefined ||
        pricing.packageUnit !== undefined
    ) {
            ...
            if (data.storePricing && data.storePricing.length > 0) {
                for (const pricing of data.storePricing) {
                    ...
                    if (newSavedFoodId !== undefined) {
                        await db.storePricing.add({
                            ...pricingWithoutId,
                            savedFoodId: newSavedFoodId,
                        } as StorePricing);
                    }
                }
            }
...
export async function getMealTemplateWithItems(
    ...
    { template: MealTemplate; items: (MealTemplateItem & { savedFood: SavedFood })[] } | undefined
> {
...
export async function getDayTemplateWithMeals(
    id: number,
): Promise<
    | { template: DayTemplate; meals: (DayTemplateMeal & { mealTemplate: MealTemplate })[] }
    | undefined
> {
...
export async function applyMealTemplateToLog(
