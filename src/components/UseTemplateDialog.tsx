...
export function UseTemplateDialog({ open, onClose, date, onApplied }: UseTemplateDialogProps) {
  ...
  const [selectedMealTemplate, setSelectedMealTemplate] = useState<MealTemplateWithNutrition | null>(null);
  const [selectedDayTemplate, setSelectedDayTemplate] = useState<DayTemplateWithNutrition | null>(null);
            ...
            {loading ? (
              ...
              <Card className="border-dashed">
                <CardContent className="py-6 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">Create templates from the Templates page.</p>
                </CardContent>
              </Card>
            ) : (
              <>
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {template.itemCount} food{template.itemCount !== 1 ? 's' : ''}
                            </p>
                          </div>
                  ...
                  >
                    <CardContent className="py-3">
                      ...
                      <div className="flex flex-wrap gap-1">
                        <Badge variant="outline" className="text-xs" style={{ borderColor: 'var(--macro-protein)', color: 'var(--macro-protein)' }}>P: {template.totalProtein}g</Badge>
                        <Badge variant="outline" className="text-xs" style={{ borderColor: 'var(--macro-carbs)', color: 'var(--macro-carbs)' }}>C: {template.totalCarbs}g</Badge>
                        <Badge variant="outline" className="text-xs" style={{ borderColor: 'var(--macro-fat)', color: 'var(--macro-fat)' }}>F: {template.totalFat}g</Badge>
                      </div>
                    </CardContent>
                  </Card>
                ))}
            ...
            <p className="text-sm">
              ...
              {activeTab === 'meal' ? (
                <>
                  ...
                  <strong>{MEAL_OPTIONS.find((o) => o.number === mealNumber)?.label}</strong>
                </>
              ) : (
                <>
