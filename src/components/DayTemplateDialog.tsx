...
export function DayTemplateDialog({ open, onClose, editingTemplate }: DayTemplateDialogProps) {
    ...
    const [mealNutrition, setMealNutrition] = useState<
        Map<number, { calories: number; protein: number; carbs: number; fat: number }>
    >(new Map());
        ...
        if (open) {
            ...
            if (editingTemplate) {
                ...
                const slots: MealSlot[] = MEAL_SLOTS.map((slot) => {
                    const existing = editingTemplate.meals.find(
                        (m) => m.mealNumber === slot.number,
                    );
                    return {
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        {mealTemplates.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {MEAL_SLOTS.map((slot) => {
                                    const currentSlot = mealSlots.find(
                                        (s) => s.mealNumber === slot.number,
                                    );
                                    return (
                                        <div key={slot.number} className="flex items-center gap-3">
                                            ...
                                            <Select
                                                value={
                                                    currentSlot?.mealTemplateId?.toString() ??
                                                    'none'
                                                }
                                                onValueChange={(value) =>
                                                    handleSlotChange(slot.number, value)
                                                }
                                            >
