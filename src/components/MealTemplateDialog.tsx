  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {editingTemplate ? 'Edit Meal Template' : step === 'name' ? 'New Meal Template' : 'Add Foods'}
          </DialogTitle>
        </DialogHeader>
              ...
              {items.length > 0 && (
                <div className="space-y-2">
                    ...
                    {items.map((item, index) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm truncate">{item.savedFood.name}</p>
                        </div>
                        <Input
                          type="number"
                          value={item.servingSize}
                          onChange={(e) =>
                            handleServingSizeChange(index, parseFloat(e.target.value) || 0)
                          }
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    ...
                    <Badge variant="outline" style={{ borderColor: 'var(--macro-protein)', color: 'var(--macro-protein)' }}>P: {Math.round(totals.protein)}g</Badge>
                    <Badge variant="outline" style={{ borderColor: 'var(--macro-carbs)', color: 'var(--macro-carbs)' }}>C: {Math.round(totals.carbs)}g</Badge>
                    <Badge variant="outline" style={{ borderColor: 'var(--macro-fat)', color: 'var(--macro-fat)' }}>F: {Math.round(totals.fat)}g</Badge>
                  </div>
                </div>
              )}
                ...
                <ScrollArea className="h-48">
                  {savedFoods.length === 0 ? (
                    <div className="text-center py-4 text-muted-foreground">
                      ...
                      <p className="text-xs">Add foods from the "My Foods" page first.</p>
                    </div>
                  ) : (
                    <div className="space-y-1 pr-4">
                      {savedFoods.map((food) => {
                        const isAdded = items.some((i) => i.savedFoodId === food.id);
                        return (
                          <Card
                            key={food.id}
