...
export function ProgressChart({ entries, goals }: ProgressChartProps) {
  ...
  const days: { date: string; label: string; calories: number; isToday: boolean; status: 'under' | 'close' | 'over' }[] = [];
    ...
    <div className="h-64">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={days} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
          ...
          >
            {days.map((day, index) => (
              <Cell key={`cell-${index}`} fill={getBarColor(day.status, day.isToday)} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
